...
It is often of interest to obtain bounds on the tails of a random variable, or two-sided inequalities, which guarantee that the random variable is close to its mean or median. These slides follow the structure of chapter 2 of \citet{wainwright2019high} and chapters 1 and 2 of \citet{vershynin2018high} to shed light on elementary techniques for obtaining \textcolor{red}{deviation} and \textcolor{red}{concentration} inequalities.\justifying
{\vskip 0.5em}
Concentration inequalities generally take the form of\justifying 
\[
P[\lvert X-\mu\rvert\geq t]\leq something\;small
\]
One way of controlling a tail probability $P[X\geq t]$ is by controlling the moments of the random variable $X$, where \textcolor{red}{by controlling higher-order moments of the variable $X$, we can obtain sharper bounds on tail probabilities}. This motivates the "Classical bounds" section of the notes.\justifying 
{\vskip0.5em}
We then extend the derivation of bounds to more general functions of the random variables in the "Martingale-based methods" section using \textcolor{red}{martingale decompositions}, as opposed to limiting the techniques to deriving bounds on \textcolor{red}{the sum of independent} random variables.\justifying  
{\vskip  0.5em}
Finally, we conclude this with a classical result on the concentration properties of Lipschitz functions of Gaussian variables.\justifying 
\end{frame}
...
&\geq tP[X\geq t]
\end{align*}
Thus, dividing both sides by $t$ yields Markov's inequality.\justifying
{\vskip 0.5em}
It is immediately obvious that Markov's inequality \textcolor{red}{requires only the existence of the first moment}. If the random variable $X$ also has finite variance - i.e. $\text{var}(X)<\infty$, we have \textcolor{red}{Chebyshev's inequality}:\justifying
...
\textbf{Proof:}
Chebyshev's inequality follows from Markov's inequality, by considering the variable $(X-\E[X])^2$ and the constant $t^2$. By substituting these in the Markov inequality, we get\justifying
\begin{align*}
...
{\vskip 0.5em}
\begin{itemize}
\item[1)]Two independent sub-Gaussian variables $X_1$ and $X_2$ possess the sub-Gaussian parameters $\sigma_1$ and $\sigma_2$ respectively. What is the sub-Gaussian parameter of $X_1+X_2$?\justifying
{\vskip 0.5em}
\item[2)] Now once again consider the sub-Gaussian tail bound (\ref{eq: Sub-Gaussian TB}). How is this result extended to the variable $X_1+X_2$?\justifying
\end{itemize}
\end{frame}
